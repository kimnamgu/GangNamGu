...
public class CommonLib {
	...
	static Log commonLog = LogFactory.getLog("SOSONG.COMMON");
	...
	public static String getStringForCLOB(Clob clob) {
		...
		try {
			...
			LogFactory.getLog("SOSONG.ERROR").debug("Failed to create String object from CLOB  >>> [ " + e + " ]");
		}finally{
			if(br!=null)
			try {
				br.close();
			} catch (IOException e) {
				LogFactory.getLog("SOSONG.ERROR").debug("Failed to close BufferedReader object  >>> [ " + e + " ]");
			}
		}
		return sbf.toString();
