...
<script type="text/javascript">
		var gfv_count = 1;
		var saup_gb = "";  //사업구분  (M:공약역점, S:스마트도시)
		...
		$(document).ready(function(){
			...
			$("#write").on("click", function(e){ //작성하기 버튼
				...
				if($('#SAUP_NM').val() == "")
				{
					alert('사업명을 오른쪽 사업리스트를 클릭하여 선택하세요.');
					$('#SAUP_NM').focus();
					return false();
				}
			
				if( $('#USER_RIGHT').val() == 'A' || $('#SAUP_DEPT_CD').val() == $('#INS_DEPT').val())	
				{				
					e.preventDefault();
					fn_insertMainSaupData();
				}
				else{
					alert('권한이 없습니다.');
				}
			...
			$("#ssaup").on("click", function(e){ //목록으로 버튼
				//alert('aa');
				e.preventDefault();
				$("#PAGE_INDEX2").val("1");
				saup_gb = "S";
			...
			$(document).keydown(function(e) {//백스페이스 키로 뒤로 가기 방지
		...
		function fn_mSaupListCallback(data){
			...
			$.each(data.list, function(key, value){										
					...
					str += "<tr class=\"list1\" onmouseover=\"this.className='list_over'\" onmouseout=\"this.className='list1'\">" +
					   "<td class=\"list_center\" nowrap>" + value.RID + "</td>" +
	                   ...
	                   "<input type=\"hidden\" name=\"JYEAR\" id=\"JYEAR\" value=\"" + NvlStr(value.SAUP_YEAR) + "\">" +
	                   "<input type=\"hidden\" name=\"JSNM\" id=\"JSNM\" value=\"" + NvlStr(value.SAUP_NM) + "\">" +
	                   "<input type=\"hidden\" name=\"JDCD\" id=\"JDCD\" value=\"" + NvlStr(value.DEPT_CD) + "\">" +
		...
		function fn_saup_info(obj, i){
			...
			tmp1 = "";
			tmp1 = obj.parent().find("#JYEAR").val();
			$("#SAUP_YEAR").val(tmp1);
		...
		function fn_insertMainSaupData(){
			...
			if($('#SAUP_DETAIL').val().length < 30)
			{
				alert('사업에 대한 설명이나 개요를 을 2-3줄 가량 입력하세요.');
				$('#SAUP_DETAIL').focus();
				return false();
			}
			...
			{
				comSubmit.setUrl("<c:url value='/daejang/insertMainSaup.do' />");
				comSubmit.submit();
			}
		}
...
<input type="hidden" id="SAUP_DAMDANG_ID" name="SAUP_DAMDANG_ID" value="${sessionScope.userinfo.userId}">
    ...
    <!--페이지 타이틀 끝 -->
	 ...
	 <tr>
		...
		<table border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td class="title"><img src="/bims/images/title_ico.gif" align="absmiddle">사업 기본데이터 등록</td>
			</tr>
		</table>
		</td>				
	</tr>
	      ...
	      <table width="100%" border="0" cellspacing="1" cellpadding="0" class="tbl" id="divMemoMainBody" style="display:block;">
	          <tr>
	            ...
	            <td width="612" colspan="3" class="tbl_list_left">
	                <input type="text" id="SAUP_NM" name="SAUP_NM" class="input" style="width:400;" readonly>
	                 <c:choose>
           			 <c:when test="${sgb eq 'M'}"> 
	                <a href="#this" id="msaup" class="bk">공약·역점 사업</a>
	                 </c:when> 
	                 <c:when test="${sgb eq 'S'}">
	                <a href="#this" id="ssaup" class="bk">스마트도시 사업</a>
	                </c:when>
	                </c:choose>	                
	            </td>
	          </tr>
	          <tr>
	            ...
	            <td width="612" colspan="3" class="tbl_list_left">
	                <textarea id="SAUP_DETAIL" name="SAUP_DETAIL" class="input" rows="10" cols="150" style="width:800px; height:50px;"></textarea>	                
	            </td>
	          </tr>
	          <tr>
	            <td width="120" class="tbl_field">예산금액</td>
	            <td width="612" colspan="3" class="tbl_list_left">
	                <input type="text" id="SAUP_BUDGET_AMT" name="SAUP_BUDGET_AMT" class="input" style="width:174;"> 천원	                
	            </td>
	          </tr>	          
	          <tr>
	            <td width="120" class="tbl_field">목표시기</td>
	            <td width="612" colspan="3" class="tbl_list_left">	                
	                <input type="text" id="SAUP_OPEN_DATE" name="SAUP_OPEN_DATE" class="input" style="width:174;" readonly> (월초나 월말로 날짜 지정하여 등록, 화면 조회시는 "월"까지만 나옴)	                
	            </td>
	          </tr>
	          <tr>
	            <td width="120" class="tbl_field">담당부서</td>
	            <td width="612" colspan="3" class="tbl_list_left"><input type="text" id="SAUP_DEPT_NM" name="SAUP_DEPT_NM" value="${sessionScope.userinfo.deptName}" class="input" style="width:174;" readonly></td>									
	          </tr>	   	          
	          <tr>
	            <td width="120" class="tbl_field">담당자</td>
	            <td width="246" class="tbl_list_left"><input type="text" id="SAUP_DAMDANG_NM" name="SAUP_DAMDANG_NM" value="${sessionScope.userinfo.userName}" class="input" style="width:174;" readonly>	            
	            <select id="GUBUN" name="GUBUN" class="input">
	            <option value="2">부서명</option>
	            <option value="1">직원명</option>
	            </select> <input type="text" id="USER_NAME" name="USER_NAME" class="input" style="width:80;"> <a href="#this" id="open" class="bk">찾기</a> <a href="#this" id="clear" class="bk">지우기</a>
	            </td>
	            <td width="120" class="tbl_field">전화번호</td>
	            <td width="246" class="tbl_list_left">
	                <input type="text" id="SAUP_DAMDANG_TEL" name="SAUP_DAMDANG_TEL" value="${sessionScope.userinfo.usertel}" class="input" style="width:100;">	                
	            </td>									
	          </tr>	                	      	       
	          <tr>
	            <td width="120" class="tbl_field">담당팀장</td>
	            <td width="246" class="tbl_list_left">
	                <input type="text" id="SAUP_DEPT_TEAMJANG" name="SAUP_DEPT_TEAMJANG" class="input" style="width:174;">	                
	            </td>
	            <td width="120" class="tbl_field">전화번호</td>
	            <td width="246" class="tbl_list_left">
	                <input type="text" id="SAUP_TEAMJANG_TEL" name="SAUP_TEAMJANG_TEL" class="input" style="width:100;">	                
	            </td>
	          </tr>
	          <tr>
	            <td width="120" class="tbl_field">담당과장</td>
	            <td width="246" class="tbl_list_left">
	                <input type="text" id="SAUP_DEPT_GWAJANG" name="SAUP_DEPT_GWAJANG" class="input" style="width:174;">	                
	            </td>
	            <td width="120" class="tbl_field">전화번호</td>
	            <td width="246" class="tbl_list_left">
	                <input type="text" id="SAUP_GWAJANG_TEL" name="SAUP_GWAJANG_TEL" class="input" style="width:100;">	                
	            </td>
	          </tr>	          		  	 
	      </table>
