...
<script type="text/javascript">
		...
		function fn_initial_setting(){
			...
			dept_cd = "${map.DEPT_CD}";
			dept_cd = dept_cd.replace(/,/g, '","');
			dept_cd = "[\"" + dept_cd + "\"]";
			
			//alert(dept_cd);
			$("#I_DEPT_CD").val(eval(dept_cd));
			$("#LAWYER_GB").val("${map.CHRG_LAWYER}");
			$("#LAST_RSLT_GB").val("${map.LAST_RSLT_GB}");						
		}
		...
		function fn_updateCrimList(){
			...
			{							
				comSubmit.setUrl("<c:url value='/incident/updateCrimList.do' />");
		...
		function fn_popJikWonListCallback(data){
			...
			$.each(data.list, function(key, value){										
					...
					str += "<tr class=\"list1\" onmouseover=\"this.className='list_over'\" onmouseout=\"this.className='list1'\">" +
	                   ...
	                   "<a href='#this' name='title' >" + NvlStr(value.USER_NAME)  + "</a>" + 					
	                   "<input type='hidden' name='JDID' id='JDID' value=" + NvlStr(value.DEPT_ID) + ">" +
					   "<input type='hidden' name='JDEPT' id='JDEPT' value=" + NvlStr(value.DEPT_NAME) + ">" +
					   "<input type='hidden' name='JID' id='JID' value=" + NvlStr(value.USER_ID) + ">" +
					   "<input type='hidden' name='JNAME' id='JNAME' value=" + NvlStr(value.USER_NAME) + ">" +
					   "<input type='hidden' name='JCID' id='JCID' value=" + NvlStr(value.CLASS_ID) + ">" +
					   "<input type='hidden' name='JCLASS' id='JCLASS' value=" + NvlStr(value.CLASS_NAME) + ">" +
					   "</td>" +	                   
	                   "<td class=\"list_center\" nowrap>" + NvlStr(value.DEPT_NAME) + "</td>" + 
	                   "<td class=\"list_center\" nowrap>" + NvlStr(value.CLASS_NAME) + "</td>" +
	                   "<td class=\"list\"'>" + NvlStr(value.GRADE_NAME) + "</td>" +	                 												
					   "</tr>" +
					   "<tr>" + 
        ...
        <!--리스트 시작 -->
	      <table width="100%" border="0" cellspacing="1" cellpadding="0" class="tbl" id="divMemoMainBody" style="display:block;">
	          <tr>
	            ...
	            <td width="243" class="tbl_list_left"><input type="text" id="INCDNT_NAME" name="INCDNT_NAME" value="${map.INCDNT_NAME}" class="input" style="width:200;"></td>
	          ...
	          <tr>
	            ...
	            <td width="244" class="tbl_list_left"><input type="text" id="SUE_DATE" name="SUE_DATE" value="<fmt:parseDate value="${map.SUE_DATE}" var="dateFmt1" pattern="yyyyMMdd"/><fmt:formatDate value="${dateFmt1}" pattern="yyyy-MM-dd"/>" class="input" style="width:100;"> <a href="#a" onclick="popUpCalendar(this, SUE_DATE, 'yyyy-mm-dd');"><img class="Lbtn" src="/sosong/images/ic_cal.gif" alt="달력" /></a></td>							
	          </tr>			 	
			  <tr>
	          ...
	          <tr>
	            ...
	            <td width="243" class="tbl_list_left"><input type="text" id="REWARD_AMT" name="REWARD_AMT" value="<fmt:formatNumber value="${map.REWARD_AMT}" pattern="###,###,###" />" class="input" style="width:100;"></td>
