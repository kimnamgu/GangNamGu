...
dhtmlxtreeObject.prototype.loadXMLString = function (xmlString) {
	this.XMLLoader.loadXMLString(xmlString);
};
/*
dhtmlxtreeObject.prototype.loadXML = function (file) {
	this.XMLLoader.loadXML(file);
};
*/
//동기화설정(async) -> true:비동기화, false:동기화
dhtmlxtreeObject.prototype.loadXML = function (file, async) {
	this.XMLLoader.loadXML(file, async);
};
...
dhtmlxtreeObject.prototype._correctPlus = function (itemObject) {
	...
	if (!itemObject.treeNod.treeLinesOn) {
		itemObject.htmlNode.childNodes[0].childNodes[0].childNodes[0].childNodes[0].src = this.imPath + workArray[3];
	} else {
...
dhtmlxtreeObject.prototype.onRowClick2 = function () {
	if (this.parentObject.treeNod.dblclickFuncHandler) {
		if (!this.parentObject.treeNod.dblclickFuncHandler(this.parentObject.id)) {
			return 0;
		}
	}
...
dhtmlxtreeObject.prototype.onRowClick = function () {
	if (this.parentObject.treeNod.openFuncHandler) {
		if (!this.parentObject.treeNod.openFuncHandler(this.parentObject.id, this.parentObject.treeNod._getOpenState(this.parentObject))) {
			return 0;
		}
	}
	if ((this.parentObject.closeble) && (this.parentObject.closeble != "0")) {
...
dhtmlxtreeObject.prototype._createItem = function (acheck, itemObject, mode) {
	...
	img.border = "0";
	if (!mode) {
		img.src = this.imPath + this.imageArray[0];
	}
	...
	// 2007년 2월 13일 이종일 -  부서 검색 후 선택된 node를 찾기위해 추가
	itemObject.span.id = itemObject.id;
	itemObject.span.tabIndex = "0";
	// 끝
...
dhtmlxtreeObject.prototype._drag = function (sourceHtmlObject, dhtmlObject, targetHtmlObject) {
	...
	if ((!this.dragMove) || (this.dragMove())) {
		this._moveNode(sourceHtmlObject.parentObject, targetHtmlObject.parentObject);
	}
};
