...
function viewUser(){
	...
	var params = '';
	var reqformno = document.frm.reqformno;
	var reqseq = document.frm.reqseq;
	
	params = 'reqformno=' + encodeURIComponent(reqformno.value)+ '&reqseq=' + encodeURIComponent(reqseq.value); 

	var userList = new Ajax.Request('/commreqformView.do', {method: 'post', postBody: params, onComplete: parseGumToUser});
	return false;
}
/**
...
function parseGumToUser(httpXML){
	...
	if(dataList.length == 1 && dataList[0].nodeName == "error") {
...
function validation(){
	...
	var url = '/commreqformInsert.do';
	var params = '';
	var gumToUser = document.frm.gumTo;
	var sungInUser = document.frm.sungIn;
	var reqformno = document.frm.reqformno.value;
	var reqseq = document.frm.reqseq.value;
	...
	//	if(sungInUser.options.length<=0){
	//	
	//		var cnf = window.confirm("담당자 전결처리 하시겠습니까?");
	//		
	//		if(!cnf) {
	//			return;
	//		} 
	...
	params = 'cmd=INSERT' + '&reqformno='+ reqformno + '&reqseq=' + reqseq + '&idList=' + encodeURIComponent(idList.join(':')) 
...
function callBack(resXML){
	...
	if (cmd == 'INSERT'){
		...
		if (parseInt(retCode, 10) > -1){
			//alert('결재선지정  저장이 완료되었습니다.');			
			self.close();
		} else if (parseInt(retCode, 10) == -1){
