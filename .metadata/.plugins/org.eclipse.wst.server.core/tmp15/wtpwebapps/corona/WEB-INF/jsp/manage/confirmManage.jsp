...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#sample_confirm").on("click", function(e){ //다운로드
			e.preventDefault();						
    ...
    function delChkRow() {
		...
		if (confirm("선택 항목을 삭제 하시겠습니까?")) {
			...
			$.ajax({
		        type: "post",
		        url : "/corona/manage/deleteConfirmChk.do",
		        data: {
		        	val:checkedValue
		        },
	...
	//수정화면가기
	function fn_updateConfrim(UID) {
		var comSubmit = new ComSubmit();
		
		comSubmit.setUrl("<c:url value='/write/updateConfirmView.do' />");
	...
	function fn_initial_setting(){
		...
		$("#NAME").val("");
		$("#CONFIRM_AREA_LIST").val("${SHOW_LIST}");
	}
	
	
	function fn_selectConfirmList(pageNo){
		var comAjax = new ComAjax();
		comAjax.setUrl("<c:url value='/manage/selectConfirmList.do' />");
		...
		comAjax.setCallback("fn_selectConfirmListCallback");
		
		comAjax.ajax();
	}
	
	
	function fn_selectConfirmListCallback(data){
		...
		if(total == 0){
			var str = "<tr>" + 
				  "<td colspan='28' style='text-align:center;'>조회된 결과가 없습니다.</td>" +
				  "</tr>";
			body.append(str);
		}
		else{
			...
			$.each(data.list, function(key, value){
				       ...
				       str += "<tr class='" + tr_class + "'>" + 
								   ...
								   "	<a href='javascript:void(0);' onclick='fn_updateConfrim("+ value.ID +")' name='PATIENT_NAME'>" + NvlStr(value.PATIENT_NAME) + "</a>"+
								   "</td>" +
								   ...
								   "<td align='center' style='color:#444444;'>" + NvlStr(value.INFECT_JIPDAN_GUBUN) + "</td>" +
								   "<td align='center' style='color:#444444;'>" + NvlStr(value.INFECT_JIPDAN_GUBUN_FACILITY) + "</td>" +
								   ...
								   "<td align='center' style='color:#444444;'>" + NvlStr(value.TA_CONFIRM_YN) + "</td>" +
	...
	function fn_updateConfirmShow(){
		var comAjax = new ComAjax();
		comAjax.setUrl("<c:url value='/manage/updateConfirmShow.do' />");
		comAjax.addParam("CONFIRM_AREA_LIST", $("#CONFIRM_AREA_LIST").val());			
		
		comAjax.setCallback("fn_updateConfirmShowCallback");
		
		comAjax.ajax();
	}
	
	
	function fn_updateConfirmShowCallback(){
       alert("저장되었습니다.");
	}
...
<div style="width:100%;padding-left:20px;padding-right:10px;float:left;">
	<div class="card" style="width:680px;float:left;">
		<div class="card-header" style="background-color:#0B2161;color:white;margin-bottom:10px;">검색</div>
		
	<form action="" method="post">
		<div class="card-body card-block" style="padding:5px;float:left;">
				<div style="float:left;">
					<div class="form-group" style="margin-bottom:5px;width:660;">
						<div class="input-group">
							<div class="input-group-addon" style="width:80px;">확진자명</div>
	...
	<div class="card" style="width:320px;">
		<div class="card-header" style="background-color:#0B2161;color:white;margin-bottom:10px;">
			확진지 노출 리스트
		</div>
		<div class="card-body card-block" style="padding:5px;">
			<div style="float:left;">
				<div class="input-group">
					<textarea style='width:300px;' class="form-control" id="CONFIRM_AREA_LIST" name="CONFIRM_AREA_LIST" rows="3"></textarea>
				</div>
			</div>
		</div>
		...
		<div class="form-actions form-group">
			<div style="padding-left:5px;padding-bottom:10px;float:left;">
				<a href='javascript:void(0);' onclick='fn_updateConfirmShow()' class="btn btn-warning"><i class="fa fa-check" ></i> 저장</a>
				<font color='#DF0174'>* 노출 리스트를 콤마(,)로 구분</font>
			</div> 
		</div>
	</div>
	...
	<div style="float:left;margin-left:10px;margin-bottom:10px;">
		...
		<form id="excelDown" name="excelDown" method="post" enctype="multipart/form-data">
			...
			<input type="image" src="/corona/images/common/btn_excel.gif" style="border:0;" onclick="doExcelDownloadProcess('/corona/manage/downloadConfirmExcelFile.do')" alt="엑셀다운로드">
		</form>
	</div>

	<!-- 샘플다운로드 -->
	<div style="float:left;margin-left:10px;margin-bottom:10px;">
		<input class="btn btn-info btn-sm" type="button" id="sample_download" onClick="fn_sampleDownload('confirm')" value="엑셀샘플"> 
	</div>

<div class="contents" style="float:right;margin-right:22px;">
	<a href="/corona/write/confirmWrite.do" class="btn btn-primary btn-sm" id="insert">입력</a>
...
<div class="contents" style="float:right;margin: 0 5 0 5;">
	...
	<!-- <a href="#this" class="btn btn-info btn-sm" id="sample_confirm"><font color="white">엑셀샘플양식 다운로드</font></a> -->
</div>	
<div class="contents" style="float:right;">

	<form id="excelUploadForm" name="excelUploadForm" enctype="multipart/form-data" method="post" action="/corona/manage/confirmManageExcelUp.do">
