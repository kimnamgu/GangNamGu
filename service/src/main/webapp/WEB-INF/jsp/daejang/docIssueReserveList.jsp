...
<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$("input[name=JOB_GB]").change(function() {
				fn_selectDocIssueReserveList(1);
			});
		...
		function fn_initial_setting(){
			...
			$('input:radio[id=DONE2_YN]').prop("checked", true);
		}
	  ...
	  function chartDrow(statisticsList){
        ...
        function drawDashboard() {
          ...
          data.addColumn('number'   , '수입증지');
 
          //그래프에 표시할 데이터
          var dataRow = [];
    		   
    	$.each(statisticsList,function(index,item){
		    dataRow = [new Date(item.process_date.substring(0,4), item.process_date.substring(4,6)-1, item.process_date.substring(6,8) ,'00' ),item.total_cnt, item.issue_yn_cnt, item.disposal_yn_cnt , item.done_yn_cnt, item.job_cnt];
		    data.addRow(dataRow);
        });
		...
		function fn_processDocIssueReserve(obj, workgb){
			...
			$.ajax({
			        type:"POST",
			        url:"/service/api/v1/processDocIssueReserve.do",
			        data : jsonData,
			        dataType : "json",
			        contentType : "application/json; charset=UTF-8",			        
			        success : function(data, stat, xhr) {
		...
		function fn_selectDocIssueReserveList(pageNo){
			...
			comAjax.setUrl("<c:url value='/api/v1/selectDocIssueReserveViewList.do' />");
			...
			comAjax.addParam("job_gb", $("input:radio[name='JOB_GB']:checked").val());	
		...
		function fn_selectDocIssueReserveListCallback(data){			
			...
			if($("input:radio[name='JOB_GB']:checked").val() == "MINWON"){
				$('#minwon_table').css("display","block");
				$('#job_table').css("display","none");
				body = $("#mytable");	
			}else{
				$('#minwon_table').css("display","none");
			...
			if(total == 0){
				...
				$.each(data.list, function(key, value){
					       ...
					       str += "<tr class='" + tr_class + "'>" + 
				   			...
				   			if($("input:radio[name='JOB_GB']:checked").val() == "MINWON"){								   			   
				   				str += "<td align='center' style='color:#444444;'>" + NvlStr(value.document_nm) + "</td>" +
				   				...
				   				str += "<td align='right' style='color:#4272d7'>" + comma(NvlStr(value.apply_unit_price)) + "</td>" +
			   			   ...
			   			   /* 민원서류 체크일때 */
				   			if($("input:radio[name='JOB_GB']:checked").val() == "MINWON"){						   			   
				   				...
				   				str += "<td width=90 align='center' style='color:#444444;word-break:break-all;'>" + NvlStr(value.visit_date) + "</td>" +
				...
				$("a[name='title2']").on("click", function(e){ //발급 
					e.preventDefault();
					confirmBox('발급으로 변경 하시겠습니까?',fn_processDocIssueReserve,$(this), "A");
				});
		...
		function pagereload() {
	    	...
	    	var jsnobj = new Object();
	    	jsnobj.job_gb =  $("input:radio[name='JOB_GB']:checked").val();
...
<div style="width:100%;padding-left:20px;padding-right:10px;">
	<div class="card" style="width:680px;display: inline-block">
		<div class="card-header" style="background-color:#0B2161;color:white;margin-bottom:10px;">검색
		
				<div class="form-check-inline form-check" style="float:right;">
	                 <label for="JOB_GB_MINWON" class="form-check-label ">
	                     <input type="radio" id="JOB_GB_MINWON" name="JOB_GB" value="MINWON" class="form-check-input" checked>민원서류 &nbsp;&nbsp;
	                 </label>
	                 <label for="JOB_GB_JOB" class="form-check-label ">
	                     &nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" id="JOB_GB_JOB" name="JOB_GB" value="JOB" class="form-check-input">&nbsp;수입증지 &nbsp;&nbsp;
	                 </label>
	             </div>
		</div>
...
<div class="col-md-12" align="center" id="minwon_table" style="display:none;">
...
<div class="col-md-12" align="center" id="job_table" style="display:none;">
	<div class="table-responsive m-b-40">
		 <table class="table table-borderless table-data3">
	     <thead style="background-color:#0B2161;">
	        <tr align="center">
