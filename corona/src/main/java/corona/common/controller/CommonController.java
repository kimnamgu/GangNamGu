...
import corona.common.service.CommonService;
import corona.common.util.Nvl;
...
public class CommonController {
	...
	public ModelAndView ssoLogin(CommandMap commandMap, HttpServletRequest request, RedirectAttributes redirecAttributes) throws Exception{
		System.out.println("ssoLogin 탐");
		ModelAndView mv = new ModelAndView("redirect:/status/mainStatusView.do");		
		String deStrId = null;
		...
		if(map.get("map") != null){
			...
			if(map2.get("map_i") == null){
				...
				mv = new ModelAndView("redirect:/gofirst.do");
	...
	//기존주소
    @RequestMapping(value="/common/jusoPopupOrg.do")
   	public ModelAndView jusoPopupOrg(CommandMap commandMap) throws Exception{
   	    	
   		ModelAndView mv = new ModelAndView("/common/jusoPopupOrg");
    ...
    //추정주소
    @RequestMapping(value="/common/jusoPopupAspect.do")
   	public ModelAndView jusoPopupAspect(CommandMap commandMap) throws Exception{
   	    	
   		ModelAndView mv = new ModelAndView("/common/jusoPopupAspect");
   		
   	   	return mv;
   	}
    
    //자가격리주소
    @RequestMapping(value="/common/jusoPopupIso.do")
   	public ModelAndView jusoPopupIso(CommandMap commandMap) throws Exception{
   	    	
   		ModelAndView mv = new ModelAndView("/common/jusoPopupIso");
   		
   	   	return mv;
   	}
    
    //접촉주소
    @RequestMapping(value="/common/jusoPopupContact.do")
   	public ModelAndView jusoPopupContact(CommandMap commandMap) throws Exception{
   	    	 
   		ModelAndView mv = new ModelAndView("/common/jusoPopupContact");
	...
	public void downloadSampleFile(CommandMap commandMap, HttpServletResponse response, HttpServletRequest request) throws Exception{
		System.out.println("downloadSampleFile 탐");
		System.out.println("commandMap 전체 확인 : " + commandMap.getMap());
		System.out.println("commandMap 확인 : " + commandMap.get("gubun"));
		
		String file_path = null;		
		
		if(request.getServerName().equals("98.23.7.110"))  //운영서버				
		{
			file_path = "/usr/local/server/file/corona/" + (String) commandMap.get("gubun") + ".xlsx";
		}
		else
		{
			file_path = "C:\\dev\\file\\corona\\" + (String) commandMap.get("gubun") + ".xlsx";
		}
		...
		response.setContentType("application/octet-stream");
		response.setContentLength(fileByte.length);
		response.setHeader("Content-Disposition", "attachment; fileName=\"" + (String) commandMap.get("gubun") + ".xlsx" +"\";");
		response.setHeader("Content-Transfer-Encoding", "binary");
		response.getOutputStream().write(fileByte);
