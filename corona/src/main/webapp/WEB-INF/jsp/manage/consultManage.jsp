...
<script type="text/javascript">
    ...
    function delChkRow() {
		...
		if (confirm("선택 항목을 삭제 하시겠습니까?")) {
			...
			$.ajax({
		        type: "post",
		        url : "/corona/manage/deleteConsultChk.do",
		        data: {
		        	val:checkedValue
		        },
	...
	//수정화면가기
	function fn_updateConsult(UID) {
		var comSubmit = new ComSubmit();
		
		comSubmit.setUrl("<c:url value='/write/updateConsultView.do' />");
	...
	function fn_selectConsultList(pageNo){
		var comAjax = new ComAjax();
		comAjax.setUrl("<c:url value='/manage/selectConsultList.do' />");
		...
		comAjax.setCallback("fn_selectConsultListCallback");
		
		comAjax.ajax();
	}
	
	
	function fn_selectConsultListCallback(data){
		...
		if(total == 0){
			var str = "<tr>" + 
				  "<td colspan='12' style='text-align:center;'>조회된 결과가 없습니다.</td>" +
				  "</tr>";
			body.append(str);
		}
		else{
			var params = {
				...
				eventName : "fn_selectConsultList"
			};
			...
			$.each(data.list, function(key, value){
				       ...
				       str += "<tr class='" + tr_class + "'>" + 
								   ...
								   "	<a href='javascript:void(0);' onclick='fn_updateConsult("+ value.ID +")' name='NAME'>" + NvlStr(value.MINWON_NAME) + "</a>"+
								   "</td>" +
								   "<td align='center' style='color:#444444;'>" + NvlStr(value.SEX) + "</td>" +
								   "<td align='center' style='color:#444444;'>" + NvlStr(value.MINWON_PHONE) + "</td>" +
								   "<td align='center' style='color:#444444;'>" + NvlStr(value.CONSULT_GUBUN) + "</td>" +
...
<div style="width:100%;padding-left:20px;padding-right:10px;">
				...
				<div style="float:left;">
					<div class="form-group" style="margin-bottom:5px;width:660;">
						<div class="input-group">
							<div class="input-group-addon" style="width:80px;">민원인명</div>
	...
	<div style="float:left;margin-left:10px;margin-bottom:10px;">
		<input class="btn btn-info btn-sm" type="button" id="sample_download" onClick="fn_sampleDownload('consult')" value="엑셀샘플"> 
	</div>


<div class="contents" style="float:right;margin-right:22px;">
	<a href="/corona/write/consultWrite.do" class="btn btn-primary btn-sm" id="insert">입력</a>
...
<div class="contents" style="float:right;">
	...
	<form id="excelUploadForm" name="excelUploadForm" enctype="multipart/form-data" method="post" action="/corona/manage/consultExcelUp.do">
