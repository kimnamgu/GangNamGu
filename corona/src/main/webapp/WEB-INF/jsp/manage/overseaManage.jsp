...
<script type="text/javascript">
    ...
    function delChkRow() {
		...
		if (confirm("선택 항목을 삭제 하시겠습니까?")) {
			...
			$.ajax({
		        type: "post",
		        url : "/corona/manage/deleteOverseaChk.do",
		        data: {
		        	val:checkedValue
		        },
	...
	//수정화면가기
	function fn_updateOversea(UID) {
		var comSubmit = new ComSubmit();
		
		comSubmit.setUrl("<c:url value='/write/updateOverseaView.do' />");
	...
	function fn_selectOverseaList(pageNo){
		var comAjax = new ComAjax();
		comAjax.setUrl("<c:url value='/manage/selectOverseaList.do' />");
		...
		comAjax.setCallback("fn_selectOverseaListCallback");
		
		comAjax.ajax();
	}
	
	
	function fn_selectOverseaListCallback(data){
		...
		if(total == 0){
			...
			var params = {
				...
				eventName : "fn_selectOverseaList"
			};
			...
			$.each(data.list, function(key, value){
				       ...
				       str += "<tr class='" + tr_class + "'>" + 
								   ...
								   "	<a href='javascript:void(0);' onclick='fn_updateOversea("+ value.ID +")' name='NAME'>" + NvlStr(value.NAME) + "</a>"+
								   "</td>" +
...
<div style="width:100%;padding-left:20px;padding-right:10px;">
				...
				<div style="float:left;">
					<div class="form-group" style="margin-bottom:5px;width:660;">
						<div class="input-group">
							<div class="input-group-addon" style="width:80px;">접촉자명</div>
	...
	<div style="float:left;margin-left:10px;margin-bottom:10px;">
		...
		<form id="excelDown" name="excelDown" method="post" enctype="multipart/form-data">
			...
			<input type="image" src="/corona/images/common/btn_excel.gif" style="border:0;" onclick="doExcelDownloadProcess('/corona/manage/downloadOverseaExcelFile.do')" alt="엑셀다운로드">
		</form>
	</div>

	<!-- 샘플다운로드 -->
	<div style="float:left;margin-left:10px;margin-bottom:10px;">
		<input class="btn btn-info btn-sm" type="button" id="sample_download" onClick="fn_sampleDownload('oversea')" value="엑셀샘플"> 
	</div>



<div class="contents" style="float:right;margin-right:22px;">
	<a href="/corona/write/overseaWrite.do" class="btn btn-primary btn-sm" id="insert">입력</a>
...
<div class="contents" style="float:right;">
	...
	<form id="excelUploadForm" name="excelUploadForm" enctype="multipart/form-data" method="post" action="/corona/manage/overseaExcelUp.do">
