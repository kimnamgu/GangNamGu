...
public class Base64
{
    ...
    public static String nullToEmptyString( String str ) {
    	return isNull( ( str == null ) ? str : str.trim() ) ? "" : str;
    }

    //Base64로 문자열 인코딩(GZIP+DONT_BREAK_LINES)
    public static String encodeString(String s) {
    	return new String(encodeBytes(nullToEmptyString(s).getBytes(), Base64.GZIP | Base64.DONT_BREAK_LINES));
    }
    
    //Base64로 숫자 문자열로 변환 후 인코딩(GZIP+DONT_BREAK_LINES)
    public static String encodeString(int i) {
    	return new String(encodeBytes(nullToEmptyString(Integer.toString(i)).getBytes(), Base64.GZIP | Base64.DONT_BREAK_LINES));
    }
    
    //Base64로 인코딩된 문자열을 원래대로 디코딩
    public static String decodeString(String s) {
    	return nullToEmptyString(new String(decode(s)));
    }
