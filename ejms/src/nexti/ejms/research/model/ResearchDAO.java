...
public class ResearchDAO
{
  ...
  public List getRchObjectResultList(Connection con, int rchno) throws Exception {
	  //logger.info("getRchObjectResultList start");
    PreparedStatement pstmt = null;
    ResultSet rs = null;
    List result = null;
    try
    ...
    //logger.info("getRchObjectResultList end");
    return result;
  }
  ...
  public List getRchSubjectResultList(Connection con, int rchno) throws Exception {
	  //logger.info("getRchSubjectResultList start");
    PreparedStatement pstmt = null;
    ResultSet rs = null;
    List result = null;
    try
    ...
    //logger.info("getRchSubjectResultList end");
    return result;
  }
    ...
    throws Exception
  {
    ...
    StringBuffer selectQuery = new StringBuffer();
    if (rchno == 0) {
      selectQuery.append("\n SELECT A.FORMSEQ, FORMGRP, FORMTITLE, FORMTYPE, SECURITY, EXAMTYPE, FILESEQ, FILENM, ORIGINFILENM, FILESIZE, EXT, ORD, NVL(REQUIRE , 'Y') AS REQUIRE, NVL(EX_FRSQ, 0) AS EX_FRSQ, NVL(EX_EXSQ, 0) AS EX_EXSQ  ");
  ...
  public List getResultSubList(int rchno, String range, RchSearchBean schbean, String sessionId)
    throws Exception
  {
	  //logger.info("getResultSubList start");
    Connection con = null;
    PreparedStatement pstmt = null;
    ResultSet rs = null;
    List rchSubList = null;
    ...
    //logger.info("getResultSubList end");
    return rchSubList;
  }
  ...
  public List getResultExamList(int rchno, String range, RchSearchBean schbean, int formseq)
    throws Exception
  {
	  //logger.info("getResultExamList start");
    Connection con = null;
    PreparedStatement pstmt = null;
    ResultSet rs = null;
    ...
    try
    {
      ...
      if (range.equals("2")) {
        ...
        //logger.info("1 . "+rchno);
        //logger.info("2 . "+formseq);
        //logger.info("3 . "+schbean.getSch_deptcd());
        //logger.info("4 . "+schbean.getSch_sex());

        if (!schbean.getSch_age().equals("")) {
          pstmt.setInt(5, Integer.parseInt(schbean.getSch_age().toString()));
          //logger.info("5 . "+schbean.getSch_age());
        }
      }
    ...
    //logger.info("getResultExamList end");
    return rchExamList;
  }
    ...
    throws Exception
  {
    ...
    insSubQuery.append("\n INSERT INTO RCHSUB_TEMP(SESSIONID, FORMSEQ, FORMGRP, FORMTITLE, FORMTYPE, SECURITY, EXAMTYPE, REQUIRE, EX_FRSQ, EX_EXSQ ) ");
    insSubQuery.append("\n VALUES (?, ?, '', '', '1', 'N', 'N', 'Y', 0, 0) ");
    ...
    throws Exception
  {
    ...
    if (rchno == 0) {
      ...
      insSubQuery = "INSERT INTO RCHSUB_TEMP(\t\t SESSIONID,\tFORMSEQ,\tFORMGRP,\tFORMTITLE,\t\t\t\t FORMTYPE, \tSECURITY, \tEXAMTYPE, \tREQUIRE, \tEX_FRSQ, \tEX_EXSQ\t ) VALUES( ?,\t\t\t?,\t\t\t?,\t\t\t?,\t\t\t\t\t ?, \t\t?,\t\t?, \t\t?,\t\t?,\t\t? ) ";
    }
    else
    {
      delSubQuery.append("\n DELETE FROM RCHSUB \t");
      ...
      insSubQuery = "INSERT INTO RCHSUB(\t\t RCHNO,\t\tFORMSEQ,\tFORMGRP,\tFORMTITLE,\t\t\t\t FORMTYPE, \tSECURITY,   \tEXAMTYPE, \tREQUIRE, \tEX_FRSQ, \tEX_EXSQ\t  ) VALUES( ?,\t\t\t?,\t\t\t?,\t\t\t?,\t\t\t\t\t ?, \t\t?,\t\t?, \t\t\t?,\t\t?,\t\t? ) ";
    }
    //logger.info(insSubQuery);
    try
    {
      ...
      if (subList != null) {
        ...
        for (int i = 0; i < subList.size(); i++) {
          ...
          if ("01".equals(subBean.getFormtype())) {
        	  pstmt.setInt(9, subBean.getEx_frsq());
        	  pstmt.setString(10, subBean.getEx_exsq());
          }else{
        	  pstmt.setInt(9, 0);
        	  pstmt.setInt(10, 0);
          }
    ...
    throws Exception
  {
    ...
    if (rchno == 0) {
      insertQuery.append("INSERT INTO RCHSUB_TEMP (SESSIONID, FORMSEQ, FORMTITLE, FORMTYPE, SECURITY, EXAMTYPE, REQUIRE, EX_FRSQ, EX_EXSQ ) ");
      insertQuery.append("VALUES(?, ?, '', '1', 'N', 'N', 'Y', 0, 0) ");
    } else {
      insertQuery.append("INSERT INTO RCHSUB (RCHNO, FORMSEQ, FORMTITLE, FORMTYPE, SECURITY, EXAMTYPE, REQUIRE, EX_FRSQ, EX_EXSQ ) ");
      insertQuery.append("VALUES(?, ?, '', '1', 'N', 'N', 'Y', 0, 0) ");
    }
    try
    {
    ...
    throws Exception
  {
    ...
    try
    {
      ...
      for (int i = 0; i < updateTable.length; i++) {
        ...
        if(i == 0){
        	sql.append("UPDATE " + updateTable[i] + " SET FORMSEQ = FORMSEQ - 1, EX_FRSQ = (CASE WHEN EX_FRSQ = ? THEN 0 ELSE EX_FRSQ END), EX_EXSQ = (CASE WHEN EX_FRSQ = ? THEN 0 ELSE EX_EXSQ END) WHERE " + kfield + " = ? AND FORMSEQ > ? ");
        	//logger.info("UPDATE " + updateTable[i] + " SET FORMSEQ = FORMSEQ - 1, EX_FRSQ = (CASE WHEN EX_FRSQ = "+formseq+" THEN 0 ELSE EX_FRSQ END), EX_EXSQ = (CASE WHEN EX_FRSQ =  "+formseq+" THEN 0 ELSE EX_EXSQ END) WHERE " + kfield + " = "+kid+" AND FORMSEQ >  "+formseq);
        }else{
        	sql.append("UPDATE " + updateTable[i] + " SET FORMSEQ = FORMSEQ - 1 WHERE " + kfield + " = ? AND FORMSEQ > ? ");
        	//logger.info("UPDATE " + updateTable[i] + " SET FORMSEQ = FORMSEQ - 1 WHERE " + kfield + " = "+kid+"AND FORMSEQ > "+formseq);
        }
    ...
    throws Exception
  {
    ...
    try
    {
      ...
      selectQuery3.append("\n SELECT '" + sessionId + "', FORMSEQ, FORMGRP, FORMTITLE, FORMTYPE, SECURITY, EXAMTYPE, REQUIRE, NVL(EX_FRSQ, 0) AS EX_FRSQ, NVL(EX_EXSQ, 0) AS EX_EXSQ ");
      selectQuery3.append("\n   FROM RCHSUB ");
      selectQuery3.append("\n  WHERE RCHNO = ? ");
   ...
   * */
	public String getRchExamList(int rchno, int formseq, String sessionId) throws Exception{
	    ...
	    try
	    {
	      StringBuffer selectQuery = new StringBuffer();
	        ...
	        selectQuery.append("SELECT SESSIONID, FORMSEQ, EXAMSEQ, EXAMCONT \n");
	        selectQuery.append("FROM RCHEXAM_TEMP \n");
	        selectQuery.append("WHERE SESSIONID = ? \n");
	        selectQuery.append("AND FORMSEQ = ? \n");
	      ...
	      pstmt.setInt(2, formseq);
	      rs = pstmt.executeQuery();
	      System.out.println(selectQuery.replace(selectQuery.indexOf("?"), selectQuery.indexOf("?")+1, "'"+sessionId+"'").replace(selectQuery.indexOf("?"), selectQuery.indexOf("?")+1, "'"+formseq+"'"));
	      ArrayList rchExamList = new ArrayList();
			while(rs.next()){
				ResearchExamBean bean = new ResearchExamBean();				
				 bean.setFormseq(rs.getInt("FORMSEQ"));
			     bean.setExamseq(rs.getInt("EXAMSEQ"));
			...
			result = getRchExamListXML(rchExamList);
    ...
    private String getRchExamListXML(ArrayList rchExamList) throws Exception {
		
		StringBuffer userListXML = new StringBuffer();

		if (rchExamList.size() > 0){
			
			ResearchExamBean bean = null;
			for (int i = 0 ; i < rchExamList.size() ; i++){
				bean = (ResearchExamBean)rchExamList.get(i);
				userListXML.append("\n	<data id=\"").append(bean.getExamseq()).append("\">");				
				userListXML.append("\n		<examdata id=\"formseq\">").append(bean.getFormseq()).append("</examdata>");
				userListXML.append("\n		<examdata id=\"examseq\">").append(bean.getExamseq()).append("</examdata>");
				userListXML.append("\n		<examdata id=\"examcont\">").append(bean.getExamcont()).append("</examdata>");
				userListXML.append("\n	</data>");
				bean = null;
			}			
		}
