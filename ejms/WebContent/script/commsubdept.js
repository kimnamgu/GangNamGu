...
function targetDeptCheck(act, type, sysdocno, sch_deptid, sch_submitsate, sch_userid, sch_inputstate) {
	try {
		var callBack = function(resultXML) {
			try {
				...
				if ( act == "ADD" ) {
					addDept(true, result.getAttribute("value"));
				} else if ( act == "DEL" ) {
					delDept(true, result.getAttribute("value"));
				}
			} catch ( exception ) {}
		}

		if ( typeof(type) == "undefined" ) type = "";
		if ( typeof(sysdocno) == "undefined" ) sysdocno = "";
		if ( typeof(sch_deptid) == "undefined" ) sch_deptid = "";
		if ( typeof(sch_submitsate) == "undefined" ) sch_submitsate = "";
		if ( typeof(sch_userid) == "undefined" ) sch_userid = "";
		if ( typeof(sch_inputstate) == "undefined" ) sch_inputstate = "";

		var url = "/targetDeptCheck.do";
		var dataUrl = url + "?type=" + type + "&sysdocno=" + sysdocno + "&sch_deptid=" + sch_deptid +
									"&sch_submitsate=" + sch_submitsate + "&sch_userid=" + sch_userid + "&sch_inputstate=" + sch_inputstate;
 ...
 * 선택된 부서를  추가한다.
 */
function addDept(isChecked, result) {
	...
	if ( typeof(isChecked) == "undefined" && isChecked != true) {
		if ( grpgbn == DEPT_GBN ) {
			targetDeptCheck("ADD", "DEPT_EXIST", document.forms[0].sysdocno.value, deptId, "", "", "");
		} else if ( grpgbn == USER_GBN ) {
			targetDeptCheck("ADD", "USER_EXIST", document.forms[0].sysdocno.value, "", "", deptId, "");
		}
		return;
	} else {
		if ( result == "true" && !document.getElementById("subdeptId").checked ) {
			alert("[" + deptFullName + "] 는(은)\n현재부서 또는 타부서의 취합대상으로 지정되어 있습니다");
			return;
		}
	}
...
function addList(deptList){
	...
	var processchk = parseInt("0" + document.forms[0].processchk.value);

	if(processchk > 4){
		//alert("문서가 마감대기 상태입니다. 취합대상을 추가할수 없습니다.");
		//return;
	}
 ...
 * 선택된 부서를 삭제한다.
 */
function delDept(isChecked, result) {
	var DeptList= document.forms[0].deptList;
	var processchk = parseInt("0" + document.forms[0].processchk.value);

	if ( DeptList.selectedIndex == -1 ) return;

	if(processchk >= 4){
		var grpgbn = DeptList.value.substr(DeptList.value.length - 2, 1);
		var deptId = DeptList.value.substr(0, DeptList.value.length - 3);
		var deptFullName = DeptList.options[DeptList.selectedIndex].text;

		if ( typeof(isChecked) == "undefined" && isChecked != true) {
			if ( grpgbn == DEPT_GBN ) {
				targetDeptCheck("DEL", "DEPT_DATA_EXIST", document.forms[0].sysdocno.value, deptId, "", "", "");
			} else if ( grpgbn == USER_GBN ) {
				targetDeptCheck("DEL", "USER_DATA_EXIST", document.forms[0].sysdocno.value, "", "", deptId, "");
			}

			if ( grpgbn != 1 ) return;
		} else {
			if ( result == "true" ) {
				alert("[" + deptFullName + "] 는(은)\n입력된 자료가 존재하여 취합대상에서 삭제할 수 없습니다");
				return;
			}
		}
	}
...
function viewDept(){
	...
	var userList = new Ajax.Request('/commsubdept/commsubdeptView.do', {method: 'post', postBody: params, onComplete: parseDept});
	return false;
}
/**
...
function validation(){
	...
	var url = '/commsubdeptInsert.do';
	var params = '';
	var deptList = document.forms[0].deptList;
	var sysdocno = document.forms[0].sysdocno.value;
	var processchk = parseInt("0" + document.forms[0].processchk.value);
	...
	if ( location.search.indexOf("reDelivery=1") == -1 ) {
		if(processchk > 1 && deptList.length == 0) {
			alert("취합대상을 지정하세요");
			return;
		}
	}
...
function callBack(resXML){
	...
	if (cmd == 'INSERT'){
		if (parseInt(retCode, 10) > 0){
			if ( opener.document.forms[0].tgtdeptnm != undefined ) {
				opener.document.forms[0].tgtdeptnm.value = subdeptList;
			}
			if ( location.search.indexOf("reDelivery=1") != -1 ) {
				opener.location.href = opener.location.href;
			}
			if ( location.search.indexOf("moveDeliComp=1") != -1 ) {
				opener.location.href = "/deliCompCollDocInfoView.do" + opener.location.search;
			}
			self.close();
		} else if (parseInt(retCode, 10) == 0){
			alert('취합대상지정 에러가 발생하였습니다.');
		}
	}
}
