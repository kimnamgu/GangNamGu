package manpower.common.controller;
...
import manpower.common.service.CommonService;
...
public class CommonController {
	...
	public void downloadFile(CommandMap commandMap, HttpServletResponse response, HttpServletRequest request) throws Exception{
		...
		{
			file_path = "/usr/local/server/file/manpower/" + dirPath + "/";
		}
		else
		{
			file_path = "C:\\dev\\file\\manpower\\" + dirPath + "\\";
		}
	...
	public void downloadSampleFile(CommandMap commandMap, HttpServletResponse response, HttpServletRequest request) throws Exception{
		...
		if(request.getServerName().equals("98.23.7.109"))  //운영서버				
		{
			file_path = "/usr/local/server/file/manpower/수의계약 업체 선정 평가 확인서.hwp";
		}
		else
		{
			file_path = "C:\\dev\\file\\manpower\\수의계약 업체 선정 평가 확인서.hwp";
		}
		...
		response.setContentType("application/octet-stream");
		response.setContentLength(fileByte.length);
		response.setHeader("Content-Disposition", "attachment; fileName=\"" + URLEncoder.encode("수의계약 업체 선정 평가 확인서.hwp","UTF-8")+"\";");
		response.setHeader("Content-Transfer-Encoding", "binary");
		response.getOutputStream().write(fileByte);
	...
	public ModelAndView excelUploadAjax(MultipartFile testFile, MultipartHttpServletRequest request) throws Exception {
		...
		view.setViewName("/manage/prvCnrtCurrentBuildList");
